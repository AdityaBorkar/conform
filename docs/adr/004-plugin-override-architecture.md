...
After ADR 002 (atomic rules, grouped display) the codebase evolved away from `Template { rules: Rule[] }` and `CheckContext` toward a plugin-composed model. `src/types.ts` now defines `Template { plugins: Plugin[], rules?: RuleOverrides }`, `Plugin<T>` with `context: (targetPath) => T` and `.defineRule({ id, name, test: ({context}) })`, and standalone `defineRule({ check: (targetPath) => CheckResult })`. Severity overrides are oxc/eslint-style (`RuleOverrides` with `"off"|"warn"|"error"|"fail"|"pass"` plus tuple form). Need to record why.
...
- **Result & output model.** `RuleResult { id, domain, files, description, status, message? }` and `ConformOutput { template, path, results, summary, groupBy? }`. Reporters filter `pass` unless `verbose`, and group by `GroupBy = "domains"|"files"` via `DOMAIN` constants.
...
- Resolver must be fixed to look at `src/presets/` (currently `templates/` — a known gotcha, `bin` path also broken, reporters not yet wired).
- `ConformConfig` gains `plugins?`/`rules?` beyond `template` (already in `src/types.ts`).
...
- **Flat `rules: Rule[]` on Template (ADR 001/002 model)** — Simple but no shared context, no domain grouping primitive, no reuse across presets.
...
- ADR 001 — Code-based templates (still accepted).
