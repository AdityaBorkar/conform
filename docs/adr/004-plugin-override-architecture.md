...
- **Plugin owns context extraction.** `Plugin<T>` is `new Plugin({ id, domain, context })` (or `definePlugin`) — `domain` is required; per-rule `domain` and `files` are explicit (rules namespace to `pluginId:ruleId`; `paramsSchema` via arktype `params` is optional). Rules inside a plugin receive typed `context` via `test({ context: T, params?: P })`; `Target` (`src/utils/fs.ts`) is the FS view passed to `context`. All Rules are defined via `Plugin#defineRule`; no standalone `defineRule` exists.
