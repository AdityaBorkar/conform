...
Always run `check:lint` before `check:types`. Run `bun test` after typecheck.
...
- `src/conform-api/index.ts` — Builder functions (`defineConfig`, `defineTemplate`, `rule`, `domain`).
- `src/conform-api/resolver.ts` — Resolves templates by **directory name** under `templates/`. Config value `template: "npm-pkg"` → looks up `templates/npm-pkg/index.ts`.
...
- **Test runner:** `bun:test` (built-in). Config in `bunfig.toml` — `root = "./tests"`, `preload = ["./tests/setup.ts"]`.
- No CI. No build step.
